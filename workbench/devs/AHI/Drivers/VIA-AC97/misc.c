...
struct CardData *AllocDriverData(struct PCIDevice *dev,
    ...
    // SB- Configure IO, if required. Force IO into 16bit/ISA address space,
    // since it
    //    apparently won't work in 32bit address range. Really ought to check
    ...
    dd->length =
        ~(ahi_pci_get_base_size(0, dd->pci_dev) & PCI_BASE_ADDRESS_IO_MASK);
...
BOOL ac97_reset(struct PCIDevice *via686b_audio) {
    ...
    // SB- Note: The 'standard' ***x reset code is quite similar to this, except
    // it
    //          de-asserts SYNC at this point, possibly in case the link/codec
    //          is in low power mode. We don't, because if we do, the dreaded
    //          half- rate problem occurs, with ~100% reliability
    //          (..interesting). It is a bit of a worry that the 'standard'
    //          reset code fails, but perhaps it is simply because we have a
    //          completely untouched link, whereas on any 686 based PC, the link
    //          was probably confugured already in the BIOS.

    outb_config(VIA_ACLINK_CTRL, VIA_ACLINK_CTRL_ENABLE | VIA_ACLINK_CTRL_RESET,
                via686b_audio);
...
int card_init(struct CardData *card) {
   ...
   int teller = 0;
    ...
    if (via686b_ACPI) {
        ...
        // SB- Enable IO (preserve other bits, but note that technically
        // speaking
        //    we should also be clreaing PSON gating here (it's already
        //    cleared))
        outb_config(0x41, (inb_config(0x41, via686b_ACPI) | 0x80),
                    via686b_ACPI);
        ...
        if (!(inb_config(0x42, via686b_ACPI) & 0x40)) {
            ...
            // We have a bail out counter, but no idea how long we should wait
            // really.
    ...
    // SB- Note: This is a possible source of trouble. It might be safer to
    // reset
    ...
    /*
       ...
       pval = inb_config(VIA_ACLINK_STAT, dev);
        ...
        if (! (pval & VIA_ACLINK_C00_READY))
        {
            ...
            pci_write_config_byte(chip->pci, VIA_ACLINK_CTRL,
       VIA_ACLINK_CTRL_INIT); udelay(100);
    ...
    */
    ...
    codec_write(card, AC97_MASTER_TONE,
...
void UpdateMonitorMixer(struct CardData *dd) {
    ...
    if (i == AC97_RECMUX_STEREO_MIX || i == AC97_RECMUX_MONO_MIX) {
        ...
        codec_write(dd, AC97_MIC_VOL, i == AC97_RECMUX_MIC ? m : mm);

        codec_write(dd, AC97_CD_VOL, i == AC97_RECMUX_CD ? s : sm);

        codec_write(dd, AC97_VIDEO_VOL, i == AC97_RECMUX_VIDEO ? s : sm);

        codec_write(dd, AC97_AUX_VOL, i == AC97_RECMUX_AUX ? s : sm);

        codec_write(dd, AC97_LINEIN_VOL, i == AC97_RECMUX_LINE ? s : sm);

        codec_write(dd, AC97_PHONE_VOL, i == AC97_RECMUX_PHONE ? m : mm);
    }
}
...
static ULONG ResetHandler(struct ExceptionContext *ctx,
                          struct ExecBase *pExecBase, struct CardData *card)
#else
