...
int __progonly_execl_wrapper(const char *path, const char *arg, ...)
{
    va_list args;
    va_start(args, arg);
...
static int system_sh(const char *string)
{
    ...
    {
        __progonly_execl_wrapper((PosixCBase->doupath ? "/bin/sh" : "bin:sh"), "sh", "-c", string, (char *) NULL);
        __progonly__Exit(127);
    }
    else
    {
        return -1;
