...
/*  FUNCTION
        ...
        \item{s} The ``wchar_t *'' argument is expected to be a pointer to an
        array of wide character type (pointer to a wide string). Characters from the
    ...
    INPUTS
        format - Format wide string as described above
    ...
    BUGS
        ...
        Because swprintf and vswprintf assume an infinitely long string,
        callers must be careful not to overflow the actual space; this is
        often impossible to assure.

    SEE ALSO
        fwprintf(), vwprintf(), vfwprintf(), swprintf(), vswprintf()
