...
VOID GDIBM__Hidd_BitMap__PutPixel(OOP_Class *cl, OOP_Object *o, struct pHidd_BitMap_PutPixel *msg)
{
    ...
    DB2(bug("[WinGDI:BitMap] hidd.bitmap.gdibitmap::PutPixel(0x%p): (%lu, %lu) = 0x%08lX\n", o, msg->x, msg->y, msg->pixel));
    Forbid();
...
{
    APTR br, orig_br;
    DB2(bug("[WinGDI:BitMap] Brush color 0x%08lX, mode 0x%08lX\n", col, mode));
...
VOID GDIBM__Hidd_BitMap__FillRect(OOP_Class *cl, OOP_Object *o, struct pHidd_BitMap_DrawRect *msg)
{
    ...
    D(bug("[WinGDI:BitMap] hidd.bitmap.gdibitmap::FillRect(0x%p, %d,%d,%d,%d)\n", o, msg->minX, msg->minY, msg->maxX, msg->maxY));
...
OOP_Object *GDIBM__Root__New(OOP_Class *cl, OOP_Object *o, struct pRoot_New *msg)
{
    ...
    D(bug("[WinGDI:BitMap] Memory device context: 0x%p\n", my_dc));
    if (my_dc) {
    ...
    D(bug("[WinGDI:BitMap] Object created by superclass: 0x%p\n", o));
    if (o) {
