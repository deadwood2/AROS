...
OOP_Object *X11BM__Root__New(OOP_Class *cl, OOP_Object *o, struct pRoot_New *msg)
{
    ...
    if (o)
    {
        ...
        data->screen = GetTagData(aHidd_BitMap_X11_SysScreen, 0, msg->attrList);
        data->cursor = (Cursor) GetTagData(aHidd_BitMap_X11_SysCursor, 0, msg->attrList);
...
VOID MNAME(Root__Get)(OOP_Class *cl, OOP_Object *o, struct pRoot_Get *msg)
{
    ...
    if (IS_X11BM_ATTR(msg->attrID, idx))
    {
        switch (idx)
        {
        case aoHidd_BitMap_X11_Drawable:
            *msg->storage = (IPTR) DRAWABLE(data);
            return;

        case aoHidd_BitMap_X11_GC:
